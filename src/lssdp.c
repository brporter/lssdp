...
#define lssdp_debug(fmt, agrs...) \
  lssdp_log(LSSDP_LOG_DEBUG, __LINE__, __func__, fmt, ##agrs)
#define lssdp_info(fmt, agrs...) \
  lssdp_log(LSSDP_LOG_INFO, __LINE__, __func__, fmt, ##agrs)
#define lssdp_warn(fmt, agrs...) \
  lssdp_log(LSSDP_LOG_WARN, __LINE__, __func__, fmt, ##agrs)
#define lssdp_error(fmt, agrs...) \
  lssdp_log(LSSDP_LOG_ERROR, __LINE__, __func__, fmt, ##agrs)
...
/** Internal Function **/
static int send_multicast_data(const char *data,
                               const struct lssdp_interface interface,
                               unsigned short ssdp_port);
...
static struct lssdp_interface *find_interface_in_LAN(lssdp_ctx *lssdp,
                                                     uint32_t address);
...
int lssdp_network_interface_update(lssdp_ctx *lssdp)
{
  ...
  const size_t SIZE_OF_INTERFACE_LIST =
      sizeof(struct lssdp_interface) * LSSDP_INTERFACE_LIST_SIZE;
  ...
  if (memcmp(original_interface, lssdp->interface, SIZE_OF_INTERFACE_LIST) ==
      0) {
...
int lssdp_socket_create(lssdp_ctx *lssdp)
{
  ...
  // set reuse address
  if (setsockopt(lssdp->sock, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) !=
      0) {
  ...
  if (setsockopt(lssdp->sock, IPPROTO_IP, IP_ADD_MEMBERSHIP, &imr,
                               ...
                               unsigned short ssdp_port)
{
  ...
  if (sendto(fd, data, data_len, 0, (struct sockaddr *)&dest_addr,
    ...
    lssdp_error("sendto %s (%s) failed, errno = %s (%d)\n", interface.name,
                interface.ip, strerror(errno), errno);
    goto end;
  }
...
static int lssdp_send_response(lssdp_ctx *lssdp, struct sockaddr_in address)
{
  ...
  // 4. send data
  if (sendto(lssdp->sock, response, response_len, 0,
